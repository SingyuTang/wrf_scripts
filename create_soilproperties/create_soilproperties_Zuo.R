...
#	   10/12/2023，官方没有将GENPARM.TBL中的refdk, refkdt, slope参数和MPTABLE.TBL中的rsurf_exp参数添加到soil_properties.nc文件中，修改后的create_soilproperties_Zuo.R对这一特性进行了修复
############################################################

# Update relevant arguments below.

#### Input geogrid:
geoFile <- "geo_em.d03.nc"
...
for (param in paramList) {
   ...
   if (!is.null(paramName)) {
      if (paramName %in% names(soltab)) {
         ...
         ncvar <- ncvar_get(ncid, param)
	 ncvar[is.na(ncvar) | is.nan(ncvar)] <- 0
